...
let inventorySelectedShopTypes = [];
const availableShopTypes = [
    "Taverna di città",
    "Taverna di strada",
    "Ostello",
    "Emporio",
            ...
            if (data.success) {
                ...
                data.items.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${item.ingombro || 0}</td>
                        <td>${item.disponibilita || ''}</td>
                        <td>${item.costo || ''}</td>
                        <td>${item.tipo || ''}</td>
                        <td>
                            <button class="btn btn-edit" onclick="openInventoryModal(${item.id})" style="background: #007bff; color: #fff; margin-right: 5px;">Modifica</button>
                            <button class="btn btn-danger" onclick="deleteInventoryItem(${item.id})">Elimina</button>
                        </td>
                    `;
...
function setupInventoryShopTypeInput() {
    const input = document.getElementById('inventoryShopTypeInput');
    const suggestions = document.getElementById('inventoryShopTypeSuggestions');

    if (!input || !suggestions) {
        console.error('Inventory shop type input elements not found');
        return;
    }
    ...
    const inputRef = document.getElementById('inventoryShopTypeInput');
    ...
    inputRef.addEventListener('input', (e) => {
        ...
        const filtered = availableShopTypes.filter(st =>
            st.toLowerCase().includes(value) && !inventorySelectedShopTypes.includes(st)
        );
        if (filtered.length > 0) {
            suggestions.innerHTML = filtered.map(st => {
                const escapedName = st.replace(/'/g, "\\'");
                return `<li onclick="addInventoryShopType('${escapedName}')">${st}</li>`;
            }).join('');
    ...
    document.addEventListener('click', (e) => {
        if (e.target !== inputRef) suggestions.style.display = 'none';
    });
}

function addInventoryShopType(name) {
    if (!inventorySelectedShopTypes.includes(name)) {
        inventorySelectedShopTypes.push(name);
        renderInventoryShopTypeTags();
    }
    document.getElementById('inventoryShopTypeInput').value = '';
    document.getElementById('inventoryShopTypeSuggestions').style.display = 'none';
}

function removeInventoryShopType(name) {
    inventorySelectedShopTypes = inventorySelectedShopTypes.filter(st => st !== name);
    renderInventoryShopTypeTags();
}

function renderInventoryShopTypeTags() {
    const container = document.getElementById('inventoryShopTypesContainer');
    container.innerHTML = inventorySelectedShopTypes.map(name => {
        const escapedName = name.replace(/'/g, "\\'");
        return `<span class="tag">${name}<span class="tag-remove" onclick="removeInventoryShopType('${escapedName}')">&times;</span></span>`;
    }).join('');
}
...
function openInventoryModal(id = null) {
    ...
    form.reset();
    inventorySelectedShopTypes = [];

    setupInventoryShopTypeInput();
    renderInventoryShopTypeTags();
        ...
        fetch(`/api/inventory`)
            ...
            .then(data => {
                ...
                if (item) {
                    ...
                    document.getElementById('inventoryItemName').value = item.inventario || '';
                    document.getElementById('inventoryItemDescription').value = item.descrizione || '';
                    document.getElementById('inventoryItemIngombro').value = item.ingombro || 0;
                    document.getElementById('inventoryItemDisponibilita').value = item.disponibilita || '';
                    document.getElementById('inventoryItemCosto').value = item.costo || '';
                    document.getElementById('inventoryItemTipo').value = item.tipo || '';

                    // Parse shop types
                    if (item.shop_types) {
                        inventorySelectedShopTypes = item.shop_types.split(',').map(s => s.trim()).filter(s => s);
                        renderInventoryShopTypeTags();
                    }
                }
            });
...
function saveInventoryItem(event) {
    ...
    const data = {
        inventario: document.getElementById('inventoryItemName').value,
        descrizione: document.getElementById('inventoryItemDescription').value,
        ingombro: parseInt(document.getElementById('inventoryItemIngombro').value) || 0,
        disponibilita: document.getElementById('inventoryItemDisponibilita').value,
        costo: document.getElementById('inventoryItemCosto').value,
        tipo: document.getElementById('inventoryItemTipo').value,
        shop_types: inventorySelectedShopTypes.join(', ')
    };
